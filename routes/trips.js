...
router.post("/", middleware.isLoggedIn, upload.single('image'), (req,res)=>{
    cloudinary.v2.uploader.upload(req.file.path, function(err, result) {
        ...
        geocoder.geocode(req.body.location, function(err,data){
            ...
            var newTrip = {name:name,price:price,image:image,description:desc,author:author,location:location,lat:lat,lng:lng};
...
router.put("/:id",middleware.checkTripOwnership, upload.single('image'),function(req,res){
    ...
    cloudinary.v2.uploader.upload(req.file.path, function(err, result) {
        ...
        geocoder.geocode(req.body.location,function(err,data){
            ...
            var lng = data[0].longitude;
            var location = data[0].formattedAddress;
            var editTrip = {name:name,price:price,image:image,description:desc,location:location,lat:lat,lng:lng,author:author};
